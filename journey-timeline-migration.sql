...
-- Update existing records to have sequence_number = 1 if NULL
...
-- Drop the new constraint if it exists (to recreate it safely)
ALTER TABLE journey_stories
DROP CONSTRAINT IF EXISTS journey_stories_story_id_node_key_sequence_key;
...
-- Add index for sequence ordering (drop first if exists)
DROP INDEX IF EXISTS idx_journey_stories_sequence;
CREATE INDEX idx_journey_stories_sequence ON journey_stories(story_id, sequence_number);
