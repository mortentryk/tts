...
export default function Game({ params, initialStoryMetadata, initialNode }: StoryPageClientProps) {
  ...
  const [startNodeKey, setStartNodeKey] = useState<string>(initialNode?.id || '1');
  ...
  // Load story by ID (only if we don't have initial data)
  useEffect(() => {
    if (!storyId) {
    ...
    loadStory();
  }, [storyId, stopSpeak, stopVoiceListening, router, nodeKeyFromUrl, nodeKeyFromParams, prefetchNode, initialNode, initialStoryMetadata]);
