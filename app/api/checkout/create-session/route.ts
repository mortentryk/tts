  ...
  return withRateLimit(request, 10, 60000, async () => {
    ...
    async (span) => {
      ...
      try {
        ...
        validation = safeValidateBody(createCheckoutSessionSchema, body);
        ...
        span?.setAttribute('checkout.type', type);
        span?.setAttribute('checkout.userEmail', userEmail);
        if (storyId) span?.setAttribute('checkout.storyId', storyId);
        if (planId) span?.setAttribute('checkout.planId', planId);
        ...
        Sentry.captureException(error, {
          tags: {
            ...
            type: validation?.data?.type || 'unknown',
          },
          extra: {
